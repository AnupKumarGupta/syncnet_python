...
for fidx, fname in enumerate(flist):
    ...
    for face in faces[fidx]:
        ...
        cv2.putText(image, 'Track %d, Conf %.3f' % (face['track'], face['conf']),
                    (int(face['x'] - face['s']), int(face['y'] - face['s'])), cv2.FONT_HERSHEY_SIMPLEX, 0.5,
                    (255, 255, 255), 2)
...
os.path.join(opt.avi_dir, opt.reference, 'video_only.avi'), os.path.join(opt.avi_dir, opt.reference, 'audio.wav'),
